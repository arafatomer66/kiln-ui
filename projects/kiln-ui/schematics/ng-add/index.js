...
function addCdkPeerDep(options) {
    return (tree, ctx) => {
...
function wireAppConfig(options) {
    return (tree, ctx) => {
        const candidates = [
        ...
        if (!path) {
            ctx.logger.warn('Could not find app.config.ts. Add `provideKilnUI()` manually:\n' +
...
function wireGlobalStyles(options) {
    return (tree, ctx) => {
        const candidates = [
        ...
        const path = candidates.find((p) => tree.exists(p));
        if (!path) {
            ctx.logger.warn("Could not find a global styles.scss or styles.css. Add manually:\n" +
...
function wireToastContainer(options) {
    return (tree, ctx) => {
        const candidates = [
